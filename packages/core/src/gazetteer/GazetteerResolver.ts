...
export class GazetteerResolver {
  ...
  resolve(point: GpsPoint): LocalitySnapshot {
    assertPoint(point);
    const country = findContainingArea(this.dataset.countries, point)?.name ?? null;
    const region = findContainingArea(this.dataset.regions, point)?.name ?? null;
...
function assertPoint({ lat, lng }: GpsPoint) {
  if (!Number.isFinite(lat) || !Number.isFinite(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
    throw new RangeError('GPS coordinates must be within latitude -90..90 and longitude -180..180.');
  }
}

function findContainingArea(areas: AdministrativeArea[], point: GpsPoint): AdministrativeArea | null {
  return areas.find((area) => isWithinBounds(area.bounds, point) && isPointInPolygon(area.rings, point)) ?? null;
}

function isWithinBounds([minLng, minLat, maxLng, maxLat]: AdministrativeArea['bounds'], point: GpsPoint) {
...
function isPointOnRing(ring: Ring, point: GpsPoint) {
  for (let i = 1; i < ring.length; i++) {
    ...
    const within = point.lng >= Math.min(x1, x2) && point.lng <= Math.max(x1, x2) && point.lat >= Math.min(y1, y2) && point.lat <= Math.max(y1, y2);
    if (Math.abs(cross) < 1e-10 && within) return true;
  }
  return false;
}

interface CityKdNode { city: GazetteerCity; axis: 'lat' | 'lng'; left: CityKdNode | null; right: CityKdNode | null }
interface NearestCity { city: GazetteerCity; distance: number }
...
function buildCityIndex(cities: GazetteerCity[]): CityKdNode | null {
  const build = (values: GazetteerCity[], depth: number): CityKdNode | null => {
    ...
    return { city: sorted[middle], axis, left: build(sorted.slice(0, middle), depth + 1), right: build(sorted.slice(middle + 1), depth + 1) };
  };
  return build(cities, 0);
}

function nearestCity(node: CityKdNode | null, point: GpsPoint, best: NearestCity | null): NearestCity | null {
  if (!node) return best;
  ...
  if (!nextBest || axisDistance <= nextBest.distance) nextBest = nearestCity(far, point, nextBest);
  return nextBest;
}
...
function searchScore(city: GazetteerCity, query: string) {
  const names = [city.name, city.asciiName, ...(city.alternateNames ?? [])].map(normalize);
