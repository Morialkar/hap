...
const dataset: GazetteerDataset = {
  ...
  countries: [{ name: 'Canada', country: null, bounds: [-80, 40, -50, 70], rings: [[[-80, 40], [-50, 40], [-50, 70], [-80, 70], [-80, 40]]] }],
  regions: [{ name: 'Québec', country: 'Canada', bounds: [-80, 45, -55, 63], rings: [[[-80, 45], [-55, 45], [-55, 63], [-80, 63], [-80, 45]]] }],
  cities: [
...
describe('GazetteerResolver', () => {
  test('records the embedded dataset version for an in-country point', () => {
    expect(new GazetteerResolver(dataset).resolve({ lat: 45.5017, lng: -73.5673 })).toEqual({
      city: 'Montréal', region: 'Québec', country: 'Canada', dataset_version: 'test-1', resolution_source: 'embedded_gazetteer',
    });
  });

  test('returns a null administrative snapshot for ocean points', () => {
    expect(new GazetteerResolver(dataset).resolve({ lat: 0, lng: -30 })).toMatchObject({ country: null, region: null, dataset_version: 'test-1' });
  });
